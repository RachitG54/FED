...
int mapOutputs(OutputMap outputMap, OutputMap outputMap2, int *vals, int m) {
	...
	for (i = 0; i < m; i++) {
		...
		printf("MAP should not FAIL %d!!!!!!!!\n", i);
		{
			unsigned char* data = (unsigned char*)&outputMap[2*i];
			int k;
			for(k=0;k<16;k++) {
				printf("%d::",data[k]);
			}
		...
		{
			unsigned char* data = (unsigned char*)&outputMap2[2*i+1];
			int k;
			for(k=0;k<16;k++) {
				printf("%d::",data[k]);
			}
		...
		{
			unsigned char* data = (unsigned char*)&outputMap2[i];
			int k;
			for(k=0;k<16;k++) {
				printf("%d::",data[k]);
			}
