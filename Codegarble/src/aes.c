...
inline unsigned char* convert128_num(__m128i var) {
    unsigned char* byte = (unsigned char*) malloc(sizeof(unsigned char)*16);
    unsigned char* bytetemp = (unsigned char*) &var;
    memcpy(byte,bytetemp,16);
    int i;
    // for(i =0; i<16;i++) {
    // 	// byte[i] = bytetemp[i];
    //     printf("%d ",(int)byte[i]);     
